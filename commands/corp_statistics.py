...
async def get_kill(session, id, hash, link, data, over):
    async with session.get(f"https://esi.evetech.net/latest/killmails/{id}/{hash}/?datasource=tranquility") as resp:
        try:
            ...
            await get_kill(session, id, hash, link, data, over)


# Function to get all kills from a zkb link
async def gather_kills(zkill_url, time_cutoff):
        ...
        while len(over) == 0 and page < 10:
            async with session.get(f"{zkill_url}page/{page}/") as response:
                ...
                if type(kills) is dict:
                    tasks = [get_kill(session, *kill, time_cutoff, data, over) for kill in kills.items()]
                else:
                    tasks = [get_kill(session, kill["killmail_id"], kill["zkb"]["hash"], time_cutoff, data, over) for
                             kill in kills]
